 ...
 * Shows enrolled athletes and allows managing drills, communication, and video uploads
 */
...
.dev-athlete-card .athlete-name { font-weight: 600; font-size: 14px; }
...
.drill-mgmt-item h4 { font-size: 14px; font-weight: 600; color: var(--text-white, #e2e8f0); }
.drill-mgmt-actions { display: flex; gap: 8px; }
    ...
    <div class="dev-athlete-list">
        ...
        <?php else: ?>
            ...
            <a href="?page=development_programs&enrollment_id=<?= (int)$a['id'] ?>" class="dev-athlete-card <?= $selected_enrollment_id == $a['id'] ? 'active' : '' ?>">
                <div class="athlete-name"><?= htmlspecialchars($a['first_name'] . ' ' . $a['last_name']) ?></div>
    ...
    <div class="dev-athlete-detail">
        ...
        <?php else: ?>
            <div class="detail-panel">
                ...
                <!-- Add Drill -->
                <h4 style="font-size:14px;font-weight:600;color:var(--text-white);margin-bottom:10px;"><i class="fas fa-plus-circle"></i> Add Drill from Library</h4>
                ...
                <!-- Assigned Drills -->
                <h4 style="font-size:14px;font-weight:600;color:var(--text-white);margin-bottom:10px;"><i class="fas fa-clipboard-list"></i> Assigned Drills (<?= count($selected_drills) ?>)</h4>
                        ...
                        <div class="drill-mgmt-item">
                            <div>
                                ...
                                <span style="font-size:11px;color:var(--text-dim);">
                                    Status: <?= str_replace('_', ' ', htmlspecialchars($sd['status'])) ?>
                                    <?php if ($sd['coach_notes']): ?> &bull; Notes: <?= htmlspecialchars(substr($sd['coach_notes'], 0, 80)) ?><?php endif; ?>
                                </span>
                            </div>
...
function addDrill(enrollmentId) {
    ...
    fetch('process_development_programs.php', {
        ...
        body: JSON.stringify({ action: 'add_drill', enrollment_id: enrollmentId, drill_id: drillId, coach_notes: notes })
    }).then(r => r.json()).then(d => { if (d.success) location.reload(); else alert(d.error || 'Failed'); }).catch(() => alert('Error'));
}
...
function removeDrill(drillAssignmentId, enrollmentId) {
    ...
    fetch('process_development_programs.php', {
        method: 'POST', headers: devHeaders,
        body: JSON.stringify({ action: 'remove_drill', drill_assignment_id: drillAssignmentId, enrollment_id: enrollmentId })
    }).then(r => r.json()).then(d => { if (d.success) location.reload(); else alert(d.error || 'Failed'); }).catch(() => alert('Error'));
}
...
function updateDrillStatus(drillAssignmentId, newStatus) {
    fetch('process_development_programs.php', {
        method: 'POST', headers: devHeaders,
        body: JSON.stringify({ action: 'update_drill_status', drill_assignment_id: drillAssignmentId, status: newStatus })
    }).then(r => r.json()).then(d => { if (d.success) location.reload(); else alert(d.error || 'Failed'); }).catch(() => alert('Error'));
}
...
function sendCoachMessage(enrollmentId) {
    ...
    }).then(r => r.json()).then(d => { if (d.success) { input.value = ''; location.reload(); } else alert(d.error || 'Failed'); }).catch(() => alert('Error'));
}
...
function sendCoachVideo(enrollmentId) {
    ...
    fetch('process_development_programs.php', {
        method: 'POST', headers: devHeaders,
        body: JSON.stringify({ action: 'send_message', enrollment_id: enrollmentId, message: 'Shared a video', video_url: videoUrl })
    }).then(r => r.json()).then(d => { if (d.success) { urlInput.value = ''; location.reload(); } else alert(d.error || 'Failed'); }).catch(() => alert('Error'));
}
</script>
