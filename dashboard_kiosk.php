...
/**
 * Kiosk Dashboard
 * Minimal POS-only interface for pos.arcticwolves.ca subdomain
...
// Require login and kiosk mode
if (!isset($_SESSION['logged_in']) || !isset($_SESSION['kiosk_mode'])) {
    header("Location: pos_kiosk.php");
    exit();
}

// Verify POS subdomain mode
if (!isset($_SESSION['pos_subdomain']) || !$_SESSION['pos_subdomain']) {
    // Not in subdomain mode, redirect to regular dashboard
    header("Location: dashboard.php?page=pos_terminal");
    exit();
}

$user_id   = $_SESSION['user_id'];
$user_role = $_SESSION['user_role'] ?? 'front_desk_staff';
...
// Redirect if user doesn't have POS access
if (!$canAccessPOS) {
    header("Location: pos_kiosk.php");
    exit();
}
...
<header class="kiosk-header">
    ...
    <div class="kiosk-user">
        <div class="kiosk-user-info">
            <div class="kiosk-user-name"><?= htmlspecialchars($user_name) ?></div>
            <div class="kiosk-user-role">POS Kiosk Mode</div>
        </div>
        <a href="logout.php" class="kiosk-logout-btn">
