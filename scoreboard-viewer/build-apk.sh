#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────
# build-apk.sh — Build the Scoreboard Viewer APK without Gradle
#
...
#
# Usage:
#   cd scoreboard-viewer && ./build-apk.sh
# ──────────────────────────────────────────────────────────────
set -euo pipefail
...
OUTPUT="$SCRIPT_DIR/scoreboard-viewer-debug.apk"

echo "╔══════════════════════════════════════════════╗"
echo "║  Scoreboard Viewer — APK Build (no Gradle)   ║"
echo "╚══════════════════════════════════════════════╝"
...
CLASSPATH="$PLATFORM:$DEPS/kotlin-stdlib-1.9.22.jar"
# shellcheck disable=SC2046
kotlinc \
  $(find "$SRC" -name "*.kt" -type f) \
  "$BUILD_DIR/gen/ca/arcticwolves/scoreboard/viewer/R.java" \
  -classpath "$CLASSPATH" \
  -d "$BUILD_DIR/classes" \
  -jvm-target 17 \
  -no-stdlib \
...
# shellcheck disable=SC2046
# The d8 tool emits "malformed kotlin.Metadata" warnings when the installed
# kotlinc version is newer than d8's bundled R8. These warnings are cosmetic
# and do not affect the output DEX or final APK. We log full output to
# build-cmdline/d8.log and only display non-warning lines to keep the build
# output clean.
"$BT/d8" \
  --release \
  --min-api 21 \
  ...
  "$DEPS/kotlin-stdlib-1.9.22.jar" 2>&1 | tee "$BUILD_DIR/d8.log" | grep -v "kotlin.Metadata\|kotlin metadata\|Should never be called\|com.android.tools.r8\|java.base/java.util.concurrent" || true
...
echo "    Package:  ca.arcticwolves.scoreboard.viewer"
