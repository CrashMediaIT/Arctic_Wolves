...
def _send_callback(callback_url: str, payload: dict):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # If the Content-Type is text/html the body may contain a PHP
            # error/warning prepended to valid JSON — try to extract it.
            body = None
            try:
                body = resp.json()
            except Exception:
                # resp.json() failed — the body may be "HTML + JSON" when
                # a PHP warning is emitted before the JSON payload.  Look
                # for the first '{' and try to parse from there.
                if resp.text:
            ...
            if body and isinstance(body, dict):
                ...
                # Completely non-JSON — include body preview for diagnostics.
                if "text/html" in content_type:
                    ...
                    logger.warning(
                        ...
                        "URL: %s — check nginx has a location /api/ block. Body: %s",
                        payload.get("job_id"), content_type, target_url, resp_preview,
                    )
                else:
                    result["response_detail"] = f"HTTP {resp.status_code} non-JSON (Content-Type: {content_type}) body={resp_preview}"
                    logger.warning("Could not parse confirmation from callback response for job %s (http=%d, ct=%s, body=%s)", payload.get("job_id"), resp.status_code, content_type, resp_preview)
            return result
        except Exception as exc:
